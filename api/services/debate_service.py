...
イベント別書き込みタイミング（設計書 §6.2）：
  ...
  complete        → sessions UPDATE（status='completed'・duration）+ question_embedding 生成（F-021）
"""
...
async def _persist_round_summary(
    ...
    col = "diversity_score_r1" if event.round == 1 else "diversity_score_r2"
    async with pool.acquire() as conn:
        await conn.execute(
            f"UPDATE deliberation_sessions SET status=$1, {col}=$2 WHERE id=$3",
            next_status,
            event.diversity_score_phi,
            session_id,
        )
...
async def _save_question_embedding(
    pool: asyncpg.Pool,
    session_id: UUID,
    question: str,
) -> None:
    """熟議完了後に question_embedding を生成して保存する（仕様書 F-021）。
    Voyage AI 呼び出し失敗は警告のみで、完了済みセッションの保存は阻害しない。
    """
    try:
        ...
        async with pool.acquire() as conn:
            await conn.execute(
                "UPDATE deliberation_sessions SET question_embedding = $1::vector WHERE id = $2",
                embedding_str,
                session_id,
            )
    except Exception:
        logger.warning("question_embedding 生成失敗（F-021 フォールバック） session_id=%s", session_id)
        ...
        async for event in stream_debate(question=question, mode=mode, memory_context=memory_context):
            ...
            try:
                if isinstance(event, SessionStartEvent):
                    ...
                    await _save_question_embedding(pool, session_id, question)
